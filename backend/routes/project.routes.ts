...
router.post("/", async (req, res) => {
    try {
        const { projectName, colorHex, description } = req.body;
        const userId = req.authUser!.userId;
        const createdProject = await createNewProject(userId, projectName, colorHex, description);
...
router.delete("/:projectId", async (req,res) => {
    try {
        ...
        if (!projectId){
            res.status(400).json({ message: "Please enter the project ID" });
        } else {
